...
import type { CommitProposal, CommitType, ConventionalAnalysis, NumstatEntry } from "./types.ts";
...
function inferTypeFromFiles(numstat: NumstatEntry[]): CommitType {
	...
	for (const entry of numstat) {
		...
		if (TEST_PATTERNS.some(pattern => lowerPath.includes(pattern))) hasTests = true;
		else if (DOC_EXTENSIONS.has(ext)) hasDocs = true;
		else if (CONFIG_EXTENSIONS.has(ext)) hasConfig = true;
		else if (STYLE_EXTENSIONS.has(ext)) hasStyle = true;
		else hasSource = true;
	}
...
export function generateFallbackAnalysis(numstat: NumstatEntry[]): ConventionalAnalysis {
	...
	return { type, scope: null, details, issueRefs: [] };
}
...
export function generateFallbackSummary(type: CommitType, numstat: NumstatEntry[]): string {
	...
	if (numstat.length === 1) return `${verb} ${file}`;
