...
import type { ConventionalGenerationConfig } from "./config.ts";
...
export class ScopeAnalyzer {
	...
	static analyzeWideChange(numstat: string): string | null {
		...
		for (const file of paths) {
			...
			const suffix = lastSuffix(file);
			if (suffix === ".md") markdown += 1;
			if (file.includes("/test") || file.includes("test_") || file.includes("_test.") || file.includes(".test.")) tests += 1;
...
/** Extract the exact scope hint string consumed by the analysis prompt. */
export function extractScopeCandidates(
	source: string | ReadonlyArray<{ path: string; additions: number; deletions: number }>,
...
function lastSuffix(file: string): string {
