 ...
 *
 * Searches talk to the search API through `gh api` because `gh search
 * prs`/`issues` quotes multi-token positional queries and returns 0 hits.
 */
...
/** Resolve a search date bound to a GitHub-search-compatible literal. */
export function parseSearchDateBound(raw: string, now: Date = new Date()): string {
	const trimmed = raw.trim();
	if (!trimmed) throw new ToolError("date bound must not be empty");
...
/** Build the GitHub-search qualifier (e.g. `created:>=2026-05-09`) for the provided bounds. */
...
export function apiCommitToSearchResult(item: GhApiSearchCommitItem): GhSearchCommitResult {
	return {
		...
		commit: item.commit
			? { author: item.commit.author ?? null, committer: item.commit.committer ?? null, message: item.commit.message }
			: null,
...
/** Matches search-query qualifiers that already scope to a repository, org, or user. */
...
export function formatSearchCommitsResults(query: string, repo: string | undefined, items: GhSearchCommitResult[]): string {
...
export async function executeSearchIssues(cwd: string, params: GithubInput, signal: AbortSignal | undefined) {
...
export async function executeSearchPrs(cwd: string, params: GithubInput, signal: AbortSignal | undefined) {
	const limit = resolveSearchLimit(params.limit);
...
export async function executeSearchCode(cwd: string, params: GithubInput, signal: AbortSignal | undefined) {
...
export async function executeSearchCommits(cwd: string, params: GithubInput, signal: AbortSignal | undefined) {
	const limit = resolveSearchLimit(params.limit);
...
export async function executeSearchRepos(cwd: string, params: GithubInput, signal: AbortSignal | undefined) {
	const limit = resolveSearchLimit(params.limit);
